import './DistrictPin.css';

type Props = {
  price: number|null; // 가격 (100만원 단위)
  handleClick: (level: number, latitude: number, longitude: number) => void // 클릭시 해당 구로 이동
  count: number; // 개수
  name: string;
  latitude: number;
  longitude: number;
...
export default function DistrictPin({
  ...
  return (
    <div
    className={`flex flex-col items-start justify-end cursor-pointer bg-transparent absolute select-none bottom-2 min-w-14 ${isGu && price === null ? 'float-animation' : 'hover:-translate-y-1 transition-transform'}`}
      onClick={() => handleClick(isGu?6:5, latitude, longitude)}
    >
      <div
